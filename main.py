...
dp = Dispatcher()  # aiogram 3.x не передаем bot в конструктор
...
async def handle_channel_post(message: types.Message):
    ...
    main_sheet.append_row([message.chat.title or "<название канала>", text])
    logging.info("Сообщение записано в Google Sheets (A, B)")

# === Main ===
async def main():
    logging.info("Бот запущен для обработки заказов и чтения канала")
    await asyncio.gather(check_new_orders(), send_alerts(), dp.start_polling(bot))
